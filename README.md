...
## 🔬 How to Update Selected Publications (`academic.html`)

To edit or add academic papers/periodicals under the **Selected Publications** section (which has expanding accordion drawers for abstracts):
1. Open `academic.html` in your text editor.
2. Search for the words `Selected Publications` or scroll down to the `<!-- Publications -->` block (around line 334).
3. Under the `<div class="space-y-8">` container, you will see publication card blocks starting with `<!-- Paper 1 -->`.

### A. To Edit an Existing Publication:
Simply change the text inside the HTML tags:
*   **Year tag**: Change the year (e.g. `2025`) inside `<span class="...">2025</span>`
*   **Genre tag**: Change the genre (e.g. `Semi-Academic Periodical`) inside the adjacent `<span>`
*   **Title**: Change the title headline inside the `<h3>` header
*   **Publisher**: Change the publisher name (e.g. `Press Information Bureau (PIB)`) inside the `<p class="... italic ...">`
*   **Description**: Change the brief summary text inside the next `<p class="... leading-relaxed">`
*   **Research Abstract**: Change the quote description inside the sliding glass card (`<p class="... leading-relaxed italic">`)

### B. To Add a New Publication:
1. Copy the entire `<!-- Paper 1 -->` code block (from `<article>` to `</article>`).
2. Paste it directly below or above the existing paper card inside the `<div class="space-y-8">` container.
3. **CRITICAL STEP (Unique Abstract ID)**: To make sure the new paper's "Read Abstract" accordion button slides open its own abstract box (and not the first paper's box), you **must change the unique ID code**:
    *   Change the button's toggle trigger ID (e.g. from `abs-1` to `abs-2`):
        `onclick="toggleAbstract('abs-2')"`
    *   Change the sliding panel's matching container ID (e.g. from `abs-1` to `abs-2`):
        `<div id="abs-2" class="abstract-panel ...">`
    *   *(If you add a third paper, use `abs-3`, a fourth paper, use `abs-4`, and so on. Both IDs must match perfectly!)*
4. Fill in your new publication details, save the file, and refresh your browser!
